...
def resnet(inputs, filters, training, strides,
                       data_format, weight_stddev, fc_size, keep_prob, num_of_params, img_sz, num_channels):
  ...
  """
  inputs = tf.reshape(inputs, shape=[-1, img_sz[0] ,img_sz[1], num_channels])
  shortcut = inputs
  ...
  inputs = conv2d_fixed_padding(
      ...
      data_format=data_format)
  inputs = batch_norm(inputs, training, data_format)
  inputs = conv2d_same(inputs, num_channels,k_h=1,k_w=1,d_h=1,d_w=1,bias=0.0)
  inputs += shortcut
  inputs = tf.nn.relu(inputs)
